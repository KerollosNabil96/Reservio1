<template>
  ...
  >
    ...
    <div class="relative">
      ...
      <div v-if="$route.path === '/'" class="absolute top-3 right-3 bg-blue-600 text-white text-xs font-bold px-2 py-1 rounded-full">
        Popular
      </div>
      <div class="absolute bottom-3 left-3">
        <span class="bg-white/90 dark:bg-gray-800/90 text-blue-600 dark:text-blue-400 font-bold px-3 py-1 rounded-full text-sm">
          {{ price }} EGP/hr
        </span>
      </div>
    </div>
    ...
    <div class="p-5 flex flex-col flex-grow">
      ...
      <div class="mt-auto flex items-center justify-between">
        ...
        <BaseButton class="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-4 py-2 rounded-md transform transition-all duration-300 hover:scale-105 shadow-md">
          <RouterLink :to="`/book-now/${id}`" class="text-white"> Book Now </RouterLink>
        </BaseButton>
      </div>
    </div>
  </div>
