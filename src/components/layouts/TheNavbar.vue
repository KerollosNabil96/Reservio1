<template>
  <nav :class="[
    ...
    <div
      ...
      <!-- Logo -->
      <RouterLink to="/" class="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">Reservio
      </RouterLink>

      <!-- Mobile Right Section (notification and hamburger) -->
      <div class="md:hidden flex items-center">
        <!-- Mobile Notification Icon -->
        <notification-dropdown v-if="isAuthenticated && !$store.state.user?.isAdmin" class="mr-2" />

        <!-- Hamburger Icon (visible on mobile, hidden on md and up) -->
        <div class="text-gray-800 dark:text-white p-2 cursor-pointer" @click="toggleMenu">
          <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
      ...
      <ul class="hidden md:flex items-center space-x-2 md:space-x-4 lg:space-x-10 justify-between">
        <li>
          <RouterLink to="/" class="nav-link hover:text-blue-600 text-base lg:text-md"
            :class="{ 'text-blue-600 active': $route.path === '/' }">{{ $t("home") }}</RouterLink>
        </li>
        <li>
          <RouterLink to="/book-now" class="nav-link hover:text-blue-600 text-base lg:text-md" :class="{
            'text-blue-600 active': $route.path.includes('/book-now'),
          }">{{ $t("book_now") }}</RouterLink>
        </li>
        <li>
          <RouterLink to="/register-venue" class="nav-link hover:text-blue-600 text-base lg:text-md" :class="{
            'text-blue-600 active': $route.path === '/register-venue',
          }">{{ $t("share_venue") }}</RouterLink>
        </li>

        <li>
          <RouterLink to="/about" class="nav-link hover:text-blue-600 text-base lg:text-md"
            :class="{ 'text-blue-600 active': $route.path === '/about' }">{{ $t("about") }}</RouterLink>
        </li>
        <li>
          <RouterLink to="/contact" class="nav-link hover:text-blue-600 text-base lg:text-md"
            :class="{ 'text-blue-600 active': $route.path === '/contact' }">{{ $t("contact") }}</RouterLink>
        </li>
        <li>
          <RouterLink to="/FAQs" class="nav-link hover:text-blue-600 text-base lg:text-md"
            :class="{ 'text-blue-600 active': $route.path === '/FAQs' }">{{ $t("faqs") }}</RouterLink>
        </li>
      </ul>
      ...
      <div class="hidden md:flex items-center space-x-4">
        ...
        <button class="flex items-center space-x-1 bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden"
          @click="toggleLanguage">
          <span class="px-3 py-1.5 transition-colors duration-300" :class="{
        ...
        <template v-if="!isAuthenticated">
          <BaseButton @click="showSigninForm = true" class="bg-blue-600 text-white px-2 lg:px-4 py-2 hover:bg-blue-700">
            {{ $t("sign_in") }}
          </BaseButton>
          <BaseButton @click="showSignupForm = true"
          ...
          <div class="relative">
            <div class="flex items-center">
              ...
              <div @click="toggleUserMenu"
                ...
                <svg class="w-4 h-4" :class="{ 'transform rotate-180': isUserMenuOpen }" fill="none"
            ...
            <!-- User Dropdown Menu -->
            <div v-if="isUserMenuOpen" v-click-outside="closeUserMenu" :class="[
              'absolute mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg py-2 z-50',
              $i18n.locale === 'ar' ? 'left-0' : 'right-0',
            ]" @click.stop>
              <RouterLink v-if="$store.state.user?.isAdmin" @click="closeUserMenu" to="/dashboard"
              ...
              <RouterLink v-else to="/profile" @click="closeUserMenu"
      ...
      <transition name="fade">
        <div v-if="isMenuOpen"
          ...
          <ul class="flex flex-col space-y-3 mb-4">
            <li>
              <RouterLink to="/" class="nav-link hover:text-blue-600" @click="closeMenu">{{ $t("home") }}</RouterLink>
            </li>
            <li>
              <RouterLink to="/register-venue" class="nav-link hover:text-blue-600" @click="closeMenu">{{
                $t("share_venue") }}
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/book-now" class="nav-link hover:text-blue-600" @click="closeMenu">{{ $t("book_now") }}
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/about" class="nav-link hover:text-blue-600" @click="closeMenu">{{ $t("about") }}
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/contact" class="nav-link hover:text-blue-600"
                :class="{ 'text-blue-600 active': $route.path === '/contact' }" @click="closeMenu">{{ $t("contact") }}
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/FAQs" class="nav-link hover:text-blue-600" @click="closeMenu">{{ $t("faqs") }}
              </RouterLink>
            </li>
          </ul>
          ...
          <div class="flex flex-col space-y-4">
            <!-- Language Toggle -->
            <button class="flex items-center justify-center space-x-1" @click="toggleLanguage">
              <span class="px-3 py-1.5 transition-colors duration-300" :class="{
                'bg-blue-600 text-white': currentLanguage === 'en',
                'text-gray-600 dark:text-gray-300 hover:text-blue-600 cursor-pointer':
                  currentLanguage === 'ar',
            ...
            <!-- Dark Mode Toggle -->
            <button class="hover:cursor-pointer flex items-center justify-center" @click="toggleDarkMode">
              <svg v-if="!isDarkMode" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                class="size-6">
                <path fill-rule="evenodd"
                  d="M9.528 1.718a.75.75 0 0 1 .162.819A8.97 8.97 0 0 0 9 6a9 9 0 0 0 9 9 8.97 8.97 0 0 0 3.463-.69.75.75 0 0 1 .981.98 10.503 10.503 0 0 1-9.694 6.46c-5.799 0-10.5-4.7-10.5-10.5 0-4.368 2.667-8.112 6.46-9.694a.75.75 0 0 1 .818.162Z"
            ...
            <template v-else>
              <div class="border-t border-gray-200 dark:border-gray-600 pt-4">
                ...
                <RouterLink v-if="$store.state.user?.isAdmin" to="/dashboard"
