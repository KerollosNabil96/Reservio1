      ...
      <div class="bg-white shadow-lg rounded-2xl p-6 w-full max-w-2xl dark:bg-gray-900">
        ...
        <div class="flex flex-col md:flex-row items-center gap-6">
          ...
          <div class="w-full md:w-1/2 space-y-4">
            <input type="text" placeholder="Payment card number" class="w-full p-2 border rounded-lg"
              v-model="cardNumber" @input="validateInput" />
            <input type="text" placeholder="Expiration date (MM/YY)" class="w-full p-2 border rounded-lg"
              v-model="cardExpiry" @input="validateInput" />
            <input type="text" placeholder="CVV" class="w-full p-2 border rounded-lg"
              v-model="cardCvc" @input="validateInput" />
          </div>
        </div>
        ...
        <div class="mt-6 flex flex-col space-y-2">
          <button @click="processPayment" :disabled="!isFormValid || loading"
            class="w-full py-2 rounded-lg font-bold"
            :class="isFormValid ? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-600 cursor-not-allowed'">
