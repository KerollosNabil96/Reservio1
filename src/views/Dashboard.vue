...
export default {
  ...
  setup() {
    ...
    const users = ref([]);
    const requests = ref([]);
    const loading = ref(false); // Define `loading` as a ref
    ...
    const confirmDelete = async () => {
      loading.value = true; // Use `loading.value` instead of `this.loading`
      try {
        const db = getDatabase();
        ...
        loading.value = false; // Use `loading.value` instead of `this.loading`
      }
    };
        ...
        try {
          const transactionResult = await runTransaction(
            ...
            (currentStats) => {
              ...
              console.log(
                `8. Updating Rejected cou          currentStats.Rejected || 0
                } to ${newRejectedCount}`
              );
  ...
  methods: {
    ...
    nextPage(type) {
      if (type === "users" && this.currentPageUsers < this.totalPagesUsers) {
        ...
        type === "requests" &&
        this.currentPageRequests < this.totalPagesRequests
      ) {
